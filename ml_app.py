...
def run_ml_app():
    ...
    a = 1
    df_map = pd.DataFrame({"lat" : [picklat, droplat], "long" : [picklong, droplong], "sz" : [a, a], "clr" : np.random.rand(2, 4).tolist()})
    arr = np.array([[picklat, picklong]])
    df = pd.DataFrame(
    arr,
    columns=['lat', 'lon'])
    st.map(df_map, latitude="lat", longitude="long", size="sz", color="clr" )
